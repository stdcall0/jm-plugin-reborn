...
export class JMComicPlugin extends Plugin {
    ...
    static readonly DEFAULT_DESCRIPTIONS: Record<string, string> = {
        id: "ID",
        name: "标题",
        page_count: "页数",
        comment_count: "评论数",
    ...
    async respond(data) {
        const fields_raw = Config.get('jmcomic')?.get('fields') || [];
        const fields_description_raw = Config.get('jmcomic')?.get('fields_description') || {};
        
        // Filter valid fields only,
        // and build fields_description: remove invalid keys and add missing defaults
        const fields = fields_raw.filter((field: string) => JMComicPlugin.VALID_FIELDS.includes(field));
        const fields_description: Record<string, string> = {};
        ...
        if (forward_msg) {
            const msg: any = await Common.makeForwardMsg(this.e, forward_msgs);
            await this.e.reply(msg);
        }
    }
